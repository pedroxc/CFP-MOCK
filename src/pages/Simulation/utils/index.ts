...
export const initialState = {
  ...
  nameAssignor: '',
  cnpjAssignor: '',
  bankMatched: { label: '', value: '' },
  valueDocument: 0,
  paymentDate: '',
  barcodeValue: '',
  typeTitle: '',
  bankCode: '',
  integerValueDocument: 0,
  hasPayment: false,
  titleType: '',
  paymentValue: 0,
  email: '',
...
export const initDate = (date: string, separator: string): Date => new Date(
  Number(date.split(separator)[2]),
  Number(date.split(separator)[1]) - 1,
  Number(date.split(separator)[0]),
);

export const maskMoneyString = (value: number): string => {
  if (value === 0) {
    return '0.00';
  }
  return value.toFixed(2).replace(/(\d)(?=(\d\d\d)+(?!\d))/g, '$1.');
};
export const formatDateToBrazilDate = (date: Date): string => `${date.getUTCDate() < 10 ? `0${date.getUTCDate()}` : date.getUTCDate()}/${date.getUTCMonth() + 1 < 10
